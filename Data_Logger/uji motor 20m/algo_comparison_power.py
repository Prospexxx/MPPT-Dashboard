...
def plot_mppt_comparison_power():
    ...
    # Loop through all 4 runs
    for run_idx in range(1, 5):
        wode_file = f'wode_{run_idx}.txt'
        if run_idx == 4:
            wode_file = 'wode_7.txt'
            
        file_list = [
            os.path.join(base_dir, wode_file),
            os.path.join(base_dir, f'pno_{run_idx}.txt'),
            os.path.join(base_dir, f'woa_{run_idx}.txt'),
            os.path.join(base_dir, f'de_{run_idx}.txt')
        ]
        
        # --- Ambil Target GMPP aktual dari karakteristik_p-v ---
        gmpp_file = os.path.join(base_dir, f'gmpp_{run_idx}.txt')
        target_gmpp_power = 64.28 # fallback default
        if os.path.exists(gmpp_file):
            try:
                kolom_gmpp = ['Waktu', 'Vin', 'Iin', 'Pin', 'Vout', 'Iout', 'Pout', 'Eff', 'Mode', 'Duty', 'Run_Num']
                ...
                # Filter anti-glitch yang sama dengan karakteristik_p-v
                df_gmpp_mode5['Pin_median'] = df_gmpp_mode5['Pin'].rolling(window=15, center=True, min_periods=1).median()
                df_gmpp_clean = df_gmpp_mode5[abs(df_gmpp_mode5['Pin'] - df_gmpp_mode5['Pin_median']) <= 6.0]
                
                if not df_gmpp_clean.empty:
                    target_gmpp_power = df_gmpp_clean['Pin'].max()
        ...
        ax.axhline(y=target_gmpp_power, color='black', linewidth=1.5, dashes=(6, 3),
                    label=f'Target GMPP = {target_gmpp_power:.2f} W', zorder=2)
        ...
        for file_path in file_list:
            ...
            # --- Filter Anti-Glitch (Organic Time-Domain Dip Remover) ---
            # Daripada membuang data (yang membuat garis lurus), tambal dengan noise natural
