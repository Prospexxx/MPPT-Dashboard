...
def plot_mppt_data_custom_time():
    ...
    base_name = os.path.basename(nama_file).lower()
    ...
    title_str = f'{algo_name} MPPT Tracking Response' if algo_name else 'MPPT Tracking Response'
    ...
    # Continuous line plot — shows MPPT adjustment smoothly
    ax1.plot(df_final['Waktu'], df_final['Duty'],
             color='#0060ad', linestyle='-', linewidth=1.5, zorder=3)
    ...
    # Continuous line plot for power
    ax2.plot(df_final['Waktu'], df_final['Pin'],
             color='#c03030', linestyle='-', linewidth=1.5, zorder=3)
    # Markers
    ax2.plot(df_final['Waktu'], df_final['Pin'],
             linestyle='none', marker='o', markersize=3,
